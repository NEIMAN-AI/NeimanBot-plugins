...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        event = await eor(event, "𝗔𝗰𝘁𝗶𝘃𝗮𝘁𝗶𝗻𝗴......")
        que[e] = []
        qeue = que.get(e)
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"{username} 𝗛𝗶𝘀 𝗠𝗼𝘁𝗵𝗲𝗿 𝗪𝗶𝗹𝗹 𝗕𝗲 𝗙𝘂𝗺𝗸𝗲𝗱 𝗡𝗼𝘄 😉")
    else:
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        c = b.first_name
        username = f"[{c}](tg://user?id={e})"
        event = await eor(event, "𝗗𝗲-𝗔𝗰𝘁𝗶𝘃𝗮𝘁𝗶𝗻𝗴.... ")
        queue = que.get(e)
        queue.pop(0)
        await event.edit(f"{username} 𝗛𝗶𝘀/𝗛𝗲𝗿 𝗠𝗼𝘁𝗵𝗲𝗿 𝗚𝗼𝘁 𝗙𝘂𝗺𝗸𝗲𝗱")
    else:
        user = event.pattern_match.group(1)
        event = await eor(event, "𝗗𝗲-𝗔𝗰𝘁𝗶𝘃𝗮𝘁𝗶𝗻𝗴.... ")
        a = await event.client.get_entity(user)
...
CmdHelp("rreplyraid").add_command(
    "rreplyraid", None, "sᴛᴀʀᴛ ʀᴇᴘʟʏ ʀʀᴀɪᴅ ᴏɴ ᴍᴇɴᴛɪᴏɴᴇᴅ ᴜsᴇʀ."
).add_command(
    "drreplyraid", None, "sᴛᴏᴘs ʀᴇᴘʟʏ ʀʀᴀɪᴅ ᴏɴ ᴍᴇɴᴛɪᴏɴᴇᴅ ᴜsᴇʀStops."
).add_warning(
    "ᴍᴀʏ ɢᴇᴛ ғʟᴏᴏᴅᴡᴀɪᴛ!!"
).add_info(
    "sᴘᴀᴍᴍᴇʀ ᴘʟᴜɢɪɴ"
