...
from TelethonNeiman.helpers.progress import progress
from TelethonNeiman.helpers.runner import runcmd
from TelethonNeiman.helpers.vids import take_screen_shot
from TelethonNeiman.utils.extras import edit_or_reply as eor
...
async def convert_to_image(event, client):
    neiman = await event.get_reply_message()
    if not (
        neiman.gif
        or neiman.audio
        or neiman.voice
        or neiman.video
        or neiman.video_note
        or neiman.photo
        or neiman.sticker
        or neiman.media
    ):
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if neiman and neiman.photo:
        neiman_final = downloaded_file_name
    elif neiman.sticker and hell.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        neiman_final = image_name20
    elif neiman.sticker and neiman.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            await eor(event, "`Unable To Fetch Shot.`")
            return
        neiman_final = imgpath
    elif neiman.gif or neiman.video or neiman.video_note:
