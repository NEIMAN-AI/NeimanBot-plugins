...
from TelethonNeiman.utils.globals import CMD_LIST, LOAD_PLUG, SUDO_LIST
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    # blacklisted chats.
    # we will not respond in these chats.
...
def on(**args):
    def decorator(func):
        ...
        Neiman.add_event_handler(wrapper, events.NewMessage(**args))
        if H2:
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            Neiman.add_event_handler(func, events.MessageEdited(**args))
